...
const oldHtml = fs.readFileSync('index_old.html', 'utf-8');
const newHtml = fs.readFileSync('index.html', 'utf-8');

const funcsToReplace = ['changeBook', 'calculateModel', 'renderApp', 'buildSimulatedOdds'];
let modifiedHtml = newHtml;

for (const func of funcsToReplace) {
  const oldCode = extractFunction(oldHtml, func);
  const newCode = extractFunction(newHtml, func);
  
  if (oldCode && newCode) {
    modifiedHtml = modifiedHtml.replace(newCode, oldCode);
    console.log('Replaced', func);
  } else {
    console.log('Could not find', func, oldCode !== null, newCode !== null);
  }
}
